...
class ReLUResNetConv2d(nn.Module):
    ...
    def __init__(self,
                 ...
                 act_norm=False,
                 act_inplace=True):
        super(ReLUResNetConv2d, self).__init__()
        self.act_norm = act_norm
        if upsampling is True:
            self.conv = nn.Sequential(*[
                nn.Conv2d(in_channels, out_channels*4, kernel_size=kernel_size,
        ...
        if (stride==1) and (upsampling==False):
            self.conv_skip = nn.Conv2d(
                in_channels, out_channels, kernel_size=1,
                stride=stride, padding=(1 - stride + 1) // 2, dilation=dilation)
    ...
    def forward(self, x):
        ...
        if (self.stride == 1) and (self.upsampling==False):
            y = self.act(self.norm(y+self.conv_skip(x)))
        return y
...
class ConvSC_ReLUResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 act_norm=True,
                 act_inplace=True):
        super(ConvSC_ReLUResNet, self).__init__()

        stride = 2 if downsampling is True else 1
        padding = (kernel_size - stride + 1) // 2

        self.conv = ReLUResNetConv2d(C_in, C_out, kernel_size=kernel_size, stride=stride,
                                upsampling=upsampling, padding=padding,
                                act_norm=act_norm, act_inplace=act_inplace)
